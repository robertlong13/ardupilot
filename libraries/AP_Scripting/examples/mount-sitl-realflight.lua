...
local function init()
  ...
  -- set camera information flags
  local cam_info = mavlink_camera_information_t()
  local vendor_name = "RealFlight"
  local model_name = "Simulated Gimbal"
  for i = 0, #vendor_name - 1 do
    cam_info:vendor_name(i, vendor_name:byte(i + 1))
  end
  for i = 0, #model_name - 1 do
  ...
  camera:set_camera_information(CAMERA_INSTANCE, cam_info)
...
local function update()
  ...
  if next_pitch_deg and next_yaw_deg then
    if yaw_is_ef then
      next_yaw_deg = wrap_180(next_yaw_deg - math.deg(ahrs:get_yaw_rad()))
    end
